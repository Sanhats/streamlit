...
def mostrar_informacion_alumno():
    with st.container(border=True):
        st.markdown('**Legajo:** 58847')
        st.markdown('**Nombre:** Sanchez Tomas Emanuel')
...
def crear_grafico_mejorado(df, producto):
    try:
        ...
        ax.set_title('Evolución de Ventas', pad=20, fontsize=14)
...
def main():
    ...
    if sucursal_seleccionada == "Todas":
        ...
        st.title(f"Análisis de Sucursal {sucursal_seleccionada}")
    ...
    for _, row in variaciones.iterrows():
        with st.container(border=True):
            ...
            col1, col2 = st.columns([1, 3])  # 25% y 75% del espacio

            with col1:
                precio_str = f"${row['precio_promedio']:,.0f}".replace(",", ".")
                margen_str = f"{row['margen_promedio']*100:.0f}%"
